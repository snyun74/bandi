...
import {
    ...
    FaPhoneAlt, FaMapMarkerAlt, FaCreditCard, FaClock, FaCheckCircle
} from 'react-icons/fa';
...
const AdminPartnerApprovalPage: React.FC = () => {
    ...
    const [filterStat, setFilterStat] = useState<'ALL' | 'R' | 'A'>('ALL');
    ...
    const filteredPartners = partnerList.filter((item) => {
        if (filterStat !== 'ALL' && item.partnerStatCd !== filterStat) {
            return false;
        }
        if (searchTerm.trim()) {
    ...
    return (
        <div className="flex flex-col h-full bg-[#F8F9FA] font-['Pretendard'] overflow-hidden">
            ...
            <div className="shrink-0 bg-white px-4 py-2.5 border-b border-gray-100 space-y-2.5">
                ...
                {/* Filter Chips with Count Summary */}
            ...
            <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3.5">
                {loading ? (
                    <div className="p-8 text-center bg-white rounded-3xl border border-gray-100 shadow-sm">
                        <p className="text-xs text-gray-400">입점사 목록을 불러오는 중입니다...</p>
                    </div>
                ) : filteredPartners.length === 0 ? (
                    <div className="p-8 text-center bg-white rounded-3xl border border-gray-100 shadow-sm space-y-1">
                        <p className="text-xs text-gray-400 font-medium">해당 조건의 입점사 내역이 없습니다.</p>
                        <p className="text-[10px] text-gray-400">거절된 건은 목록에서 자동 제외됩니다.</p>
                    </div>
                ) : (
                            ...
                            >
                                ...
                                <div className="flex justify-between items-start gap-2">
                                    <div className="flex items-center gap-2">
                                        <div className="w-10 h-10 rounded-2xl bg-gray-100 flex items-center justify-center text-[#003C48] font-extrabold text-sm shrink-0">
                                            🏢
                                        </div>
                                        <div>
                                ...
                                <div className="space-y-1.5 text-[11px] text-gray-600">
                                    ...
                                    {(item.bankNm || item.accountNo || item.accountHolderNm) && (
                                        <div className="mt-1 pt-2 pb-1 px-3 bg-gray-50 rounded-2xl border border-gray-200/80 space-y-1">
                                    ...
                                    <div className="flex justify-between items-center text-[10px] text-gray-400 pt-1.5 border-t border-gray-100">
                                        ...
                                        {item.updDtime && item.updDtime !== item.insDtime && (
                                            <span>승인/갱신: {formatDateTime(item.updDtime)}</span>
                                        )}
                                    </div>
                                </div>

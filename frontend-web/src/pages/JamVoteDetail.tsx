...
const JamVoteDetail: React.FC = () => {
    const { voteId } = useParams<{ voteId: string }>();
    const navigate = useNavigate();
    const [voteDetail, setVoteDetail] = useState<JamVoteDetail | null>(null);
    ...
    useEffect(() => {
        const fetchVoteDetail = async () => {
            ...
            try {
                ...
                const url = userId
                    ? `/api/jam-vote/${voteId}?userId=${userId}`
                    : `/api/jam-vote/${voteId}`;
    ...
    const handleCancelVote = async () => {
        ...
        try {
            const response = await fetch('/api/jam-vote/cancel', {
                method: 'POST',
            ...
            if (response.ok) {
                ...
                setIsAlertOpen(true);
                // Refresh vote detail
                const url = `/api/jam-vote/${voteId}?userId=${userId}`;
    ...
    const handleSubmit = async () => {
        ...
        try {
            const response = await fetch('/api/jam-vote/submit', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    voteId: Number(voteId),
                ...
                {voteDetail ? (
                    <div className="p-6 flex-1">
                        ...
                        <div className="flex justify-end mb-4">
                            <button
                                onClick={() => navigate(`/main/jam/vote/status/${voteId}`)}
                        ...
                        <div className="space-y-3 mb-8">
                            {voteDetail.questions[0].items.map((item) => {
                                const isSelected = selectedItems.includes(item.bnVoteItemNo);
                                const isMultiple = voteDetail.questions[0].questionType === 'MULT';
                                return (
                                    <div
                                        key={item.bnVoteItemNo}
                                        onClick={() => handleItemClick(item.bnVoteItemNo, isMultiple)}
                                        className={`

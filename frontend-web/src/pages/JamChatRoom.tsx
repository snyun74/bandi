...
interface ChatMessage {
    ...
    voteNo?: number; // Kept for interface compatibility but not used
}
...
const JamChatRoom: React.FC = () => {
    ...
    const fetchMessages = useCallback(async (lastMsgNo?: number) => {
        ...
        try {
            const url = lastMsgNo
                ? `/api/jam-chat/${roomNo}/messages?userId=${userId}&lastMsgNo=${lastMsgNo}`
                : `/api/jam-chat/${roomNo}/messages?userId=${userId}`;
        ...
        try {
            ...
            if (uploadRes.ok) {
                ...
                const msgRes = await fetch('/api/jam-chat/message', {
                    ...
                    body: JSON.stringify({
                        cnNo: Number(roomNo), // DTO expects cnNo but maps to bnNo in backend
                        sndUserId: userId,

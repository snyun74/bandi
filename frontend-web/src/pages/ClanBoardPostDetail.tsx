...
const ClanBoardPostDetail: React.FC = () => {
    ...
    const anonymousMap = React.useMemo(() => {
        ...
        comments.forEach(comment => {
            if (comment.maskingYn === 'Y' && comment.writerUserId) {
                if (comment.writerUserId === post?.writerUserId) {
                    if (!map.has(comment.writerUserId)) {
                        map.set(comment.writerUserId, '익명(글쓴이)');
                    }
                } else {
                    if (!map.has(comment.writerUserId)) {
                        map.set(comment.writerUserId, `익명${counter}`);
                        counter++;
                    }
                }
            }
    ...
    const getCommentAuthorDisplay = (comment: Comment) => {
        const isPostAuthor = comment.writerUserId === post?.writerUserId;
        if (comment.maskingYn === 'Y') {
            return anonymousMap.get(comment.writerUserId) || (isPostAuthor ? '익명(글쓴이)' : '익명');
        }
        const baseNick = comment.userNickNm || comment.writerUserId || "사용자";
    ...
    const handleBlockConfirm = async () => {
        ...
        try {
            const res = await fetch('/api/boards/block', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ userId, blockUserId: post.writerUserId })
            });
            if (res.ok) {
                setIsBlockConfirmOpen(false);
                setModalMessage(`${post.userNickNm}님을 차단했습니다.`);
                setOnModalConfirm(() => () => navigate(`/main/clan/board/${clanId}`));
                setIsModalOpen(true);
            }
        } catch (e) {
    ...
    const handleReportSubmit = async () => {
        if (!reportContent.trim() || !userId || !post) return;
        try {
    ...
    return (
        <div className="flex flex-col bg-white font-['Pretendard'] min-h-full" style={{ fontFamily: '"Pretendard", sans-serif' }}>
            ...
            <CommonModal
                ...
                type="confirm"
                message={`정말 ${post.userNickNm}님을 차단하시겠습니까?\n(차단 시 ${post.userNickNm}님이 작성한 모든 게시물은 보이지 않습니다.)`}

...
const UserAvatar: React.FC<{ userId: string; size?: number }> = ({ userId, size = 28 }) => {
    ...
    if (hasError) {
        return (
            <div style={{ width: size, height: size }} className="rounded-full bg-[#003C48] flex items-center justify-center flex-shrink-0">
                <svg style={{ width: size * 0.55, height: size * 0.55 }} fill="none" stroke="white" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
            </div>
        );
    }
    return (
        <img
            src={`/api/user/profile/${userId}`}
            alt={userId}
                        ...
                        <div key={member.cnUserId} className={`flex flex-col p-3 ${index !== activeMembers.length - 1 ? 'border-b border-gray-100' : ''}`}>
                            <div className="flex justify-between items-center">
                                <div className="flex items-center gap-2 text-[#003C48]">
                                    <UserAvatar userId={member.cnUserId} size={28} />
